...
export class DetalheCriancaComponent implements OnInit {
  ...
  formatarPeso(pesoGramas: number): string {
    ...
    return `${pesoKg.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg`;
  }

  formatarMedida(valor: number, unidade: string): string {
    return `${valor.toLocaleString('pt-BR', { minimumFractionDigits: 1, maximumFractionDigits: 2 })} ${unidade}`;
  }
  ...
  labelSexo(sexo: string | null): string {
    const labels: Record<string, string> = { FEMININO: 'Feminino', MASCULINO: 'Masculino', NAO_INFORMADO: 'Não informado' };
    return sexo ? labels[sexo] ?? sexo : 'Não informado';
  }

  labelTipoParto(tipoParto: string): string {
    const labels: Record<string, string> = { VAGINAL: 'Vaginal', CESAREA: 'Cesárea', VAGINAL_INSTRUMENTADO: 'Vaginal com instrumento', NAO_INFORMADO: 'Não informado' };
    return labels[tipoParto] ?? tipoParto;
  }

  labelTipoGestacao(tipoGestacao: string): string {
    const labels: Record<string, string> = { UNICA: 'Única', MULTIPLA: 'Múltipla', NAO_INFORMADO: 'Não informado' };
    return labels[tipoGestacao] ?? tipoGestacao;
  }

  labelStatusCondicao(status: string): string {
    const labels: Record<string, string> = { NAO: 'Não', SIM: 'Sim', EM_INVESTIGACAO: 'Em investigação', PREFIRO_INFORMAR_DEPOIS: 'Prefiro informar depois' };
    return labels[status] ?? status;
  }

  labelTriagem(status: string): string {
    const labels: Record<string, string> = { REALIZADO: 'Realizado', PENDENTE: 'Pendente', NAO_INFORMADO: 'Não informado' };
    return labels[status] ?? status;
  }

  labelAlimentacaoInicial(alimentacao: string): string {
    const labels: Record<string, string> = { ALEITAMENTO_MATERNO_EXCLUSIVO: 'Aleitamento materno exclusivo', ALEITAMENTO_MISTO: 'Aleitamento misto', FORMULA_INFANTIL: 'Fórmula infantil', NAO_INFORMADO: 'Não informado' };
    return labels[alimentacao] ?? alimentacao;
  }

  listarIntercorrencias(crianca: Crianca): string {
    const pontos = [
      crianca.utiNeonatal ? 'UTI neonatal' : '', crianca.reanimacaoNeonatal ? 'reanimação ao nascer' : '',
      crianca.ictericiaNeonatal ? 'icterícia neonatal' : '', crianca.dificuldadeRespiratoria ? 'dificuldade respiratória' : '',
  ...
  listarPontosGestacao(crianca: Crianca): string {
    const pontos = [
      crianca.diabetesGestacional ? 'diabetes gestacional' : '', crianca.hipertensaoGestacional ? 'pressão alta ou pré-eclâmpsia' : '',
      crianca.infeccaoGestacional ? 'infecção importante' : '', crianca.sangramentoGestacional ? 'sangramento importante' : '',
      crianca.usoAlcoolGestacao ? 'exposição a álcool' : '', crianca.usoTabacoGestacao ? 'exposição a tabaco' : '',
