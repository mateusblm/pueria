...
export class RegistroContextoCriancaComponent implements OnInit {
  ...
  private redefinir(): void { this.editandoId.set(''); this.humorSelecionado.set('TRANQUILO'); this.observacoesSelecionadas.set([]); this.form.reset({ dataRegistro: this.paraEntrada(this.dataMaximaIso), descricao: '' }); }
  private descricaoParaSalvar(): string {
    const descricao = this.form.controls.descricao.value?.trim() ?? '';
    if (this.contexto !== 'humor') return descricao;
