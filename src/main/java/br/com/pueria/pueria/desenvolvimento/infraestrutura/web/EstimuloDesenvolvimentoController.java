...
@RestController @RequestMapping("/api/criancas/{criancaId}/desenvolvimento/estimulos") public class EstimuloDesenvolvimentoController {
 ...
 @GetMapping("/recomendacoes") public List<EstimuloDesenvolvimentoResponse> recomendacoes(@PathVariable UUID criancaId,Authentication auth){return useCase.listarRecomendacoes(criancaId,auth.getName()).stream().map(EstimuloDesenvolvimentoResponse::de).toList();}
